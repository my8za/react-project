...
제가 진행한 기능 부분은 영화api 자료를 받아 영화 검색기능과 검색한 결과를 페이지로 나타내는 기능을 하였습니다.
저는 2가지 기능 모두 구현에 힘든부분이 있었지만 이것을 해결하기 위해
저는 우선 제가 만들려는 기능의 견본 자료를 찾고 제가 구현 하는부분과 대조를 하여
이런 부분이 있어야 된다는걸 파악을 하고 그것에 필요한 자료와 모듈 등을 수집하고 구현했습니다
